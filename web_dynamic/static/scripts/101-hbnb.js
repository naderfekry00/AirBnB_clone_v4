$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (const place of data) {
          ...
          const review_div = $('<div class="reviews"></div>');
          const review_header = $('<h2>Reviews</h2>');
          const toggle = $('<span class="toggle-review" place-id="' + place.id + '">show</span>');
          review_div.append(review_header);
          const reviewlist = $('<ul class="review-list"></ul>');
          review_div.append(toggle);
          review_div.append(reviewlist);
          article.append(review_div);
          $('section.places').append(article);
        }
      }
    });
  ...
  $(document).on('click', '.toggle-review', function () {
    ...
    if ($(this).text() === 'show') {
      $.get('http://0.0.0.0:5001/api/v1/places/' + placeID + '/reviews', function(revdata) {
        for (const review of revdata) {
          const itemReview = $('<li></li><BR/>').html(review.text.replace(/\n/g, '<BR />'));
          reviewlist.append(itemReview);
        }
      });
      $(this).text('hide');
    } else {
